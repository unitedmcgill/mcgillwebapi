...
function XMLRPC_request($site, $location, $methodName, $params = NULL, $user_agent = NULL){
	...
	$conn = fsockopen ($site, $port, $errno, $errstr, 5); #open the connection, 5 second timeout
	
	//stream_set_timeout($conn, 60*10); // timeout after 10 minutes
	...
	if(!$conn){ #if the connection was not opened successfully
		...
		stream_set_timeout($conn, 20); // timeout after 20 seconds
		...
		// Print out the raw socket data for debugging:
		//echo $response;
